...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
	...
	public List<CompilationUnit> getCompilationUnits() {
		...
		addSelectByConditionsMethod(interfaze);
		addSelectCountByConditionsMethod(interfaze);
		addDeleteLogicByConditionsMethod(interfaze);
		addDeleteLogicByPrimaryKeyMethod(interfaze);
		addUpdateByConditionsSelectiveMethod(interfaze);
	...
	protected void addSelectByConditionsMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateSelectByConditions()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new SelectByConditionsMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}

	protected void addSelectCountByConditionsMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateSelectCountByConditions()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new SelectCountByConditionsMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}

	protected void addDeleteLogicByConditionsMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateSelectCountByConditions()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new DeleteLogicByConditionsMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}

	protected void addDeleteLogicByPrimaryKeyMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateSelectCountByConditions()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new DeleteLogicByPrimaryKeyMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}

	protected void addUpdateByConditionsSelectiveMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateSelectCountByConditions()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new UpdateByConditionsSelectiveMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}
