...
public class XMLMapperGenerator extends AbstractXmlGenerator {
	...
	protected XmlElement getSqlMapElement() {
		...
		addSelectByConditionsElement(answer);
		addSelectCountByConditionsElement(answer);
		addDeleteLogicByConditionsElement(answer);
		addDeleteLogicByPrimaryKeyElement(answer);
		addUpdateByConditionsSelectiveElement(answer);
	...
	protected void addSelectByConditionsElement(XmlElement parentElement) {
		if (introspectedTable.getRules().generateSelectByConditions()) {
			AbstractXmlElementGenerator elementGenerator = new SelectByConditionsElementGenerator();
			initializeAndExecuteGenerator(elementGenerator, parentElement);
		}
	}

	protected void addUpdateByConditionsSelectiveElement(XmlElement parentElement) {
		if (introspectedTable.getRules().generateSelectByConditions()) {
			AbstractXmlElementGenerator elementGenerator = new UpdateByConditionsSelectiveElementGenerator();
			initializeAndExecuteGenerator(elementGenerator, parentElement);
		}
	}

	protected void addSelectCountByConditionsElement(XmlElement parentElement) {
		if (introspectedTable.getRules().generateSelectByConditions()) {
			AbstractXmlElementGenerator elementGenerator = new SelectCountByConditionsElementGenerator();
			initializeAndExecuteGenerator(elementGenerator, parentElement);
		}
	}

	protected void addDeleteLogicByConditionsElement(XmlElement parentElement) {
		if (introspectedTable.getRules().generateSelectByConditions()) {
			AbstractXmlElementGenerator elementGenerator = new DeleteLogicByConditionsElementGenerator();
			initializeAndExecuteGenerator(elementGenerator, parentElement);
		}
	}

	protected void addDeleteLogicByPrimaryKeyElement(XmlElement parentElement) {
		if (introspectedTable.getRules().generateSelectByConditions()) {
			AbstractXmlElementGenerator elementGenerator = new DeleteLogicByPrimaryKeyElementGenerator();
			initializeAndExecuteGenerator(elementGenerator, parentElement);
		}
	}
