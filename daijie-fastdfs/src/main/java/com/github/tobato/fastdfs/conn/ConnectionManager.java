...
public class ConnectionManager {
    ...
    protected <T> T execute(InetSocketAddress address, Connection conn, FdfsCommand<T> command) {
        try {
            ...
            try {
                if (null != conn) {
                    pool.getPool().returnObject(address, conn);
                }
            } catch (Exception e) {
    ...
    protected Connection getConnection(InetSocketAddress address) {
        ...
        try {
            // 获取连接
            conn = pool.getPool().borrowObject(address);
        } catch (FdfsException e) {
            throw e;
        } catch (Exception e) {
    ...
    public void dumpPoolInfo(InetSocketAddress address) {
        ...
        LOGGER.debug("活动连接{}", pool.getPool().getNumActive(address));
        LOGGER.debug("空闲连接{}", pool.getPool().getNumIdle(address));
        LOGGER.debug("连接获取总数统计{}", pool.getPool().getBorrowedCount());
        LOGGER.debug("连接返回总数统计{}", pool.getPool().getReturnedCount());
        LOGGER.debug("连接销毁总数统计{}", pool.getPool().getDestroyedCount());
