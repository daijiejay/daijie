...
import org.daijie.lock.redis.RedisLockAutoConfiguration;
import org.daijie.lock.zk.EnableZkLock;
...
/**
 * zookepper实现锁工具类测试，使用默认为本地zookepper服务地址
 * @author daijie
 * @since 2019/5/17
 */
@ComponentScan(excludeFilters= {@ComponentScan.Filter(type= FilterType.ASSIGNABLE_TYPE, value= {RedisLockAutoConfiguration.class})})
@EnableZkLock
@RunWith(SpringRunner.class)
@SpringBootTest(classes = ZookepperLockSpringBootTest.class)
public class ZookepperLockSpringBootTest {
    //操作公共数据，测试是否安全
    ...
    public void testLock() throws Exception {
        //不加锁
        System.out.println("不加锁执行前-increment:" + increment + "runFrequency" + runFrequency.get());
        concurrentExecute(RedisLockSpringBootTest.LockType.NONE);
        System.out.println("不加锁执行后-increment:" + increment + "runFrequency" + runFrequency.get());
        ...
        //用工具类加锁
        System.out.println("工具类加锁执行前-increment:" + increment + "runFrequency" + runFrequency.get());
        concurrentExecute(RedisLockSpringBootTest.LockType.TOOL);
        System.out.println("工具类加锁执行后-increment:" + increment + "runFrequency" + runFrequency.get());
        ...
        concurrentExecute(RedisLockSpringBootTest.LockType.ANONTATION);
     ...
     * @param lockType 加锁类型
     */
    public void concurrentExecute(RedisLockSpringBootTest.LockType lockType) throws Exception {
